...
public class SimpleMobility extends Command {
  ...
  public void initialize() {
    ...
    // System.out.println(currentPose2d.getX() + " " + currentPose2d.getY() + " " +
    // currentPose2d.getRotation().getDegrees());

    // Pose2d goal = new Pose2d(new Translation2d((currentPose2d.getX() +
    // forwardDistM), currentPose2d.getY()),
    // currentPose2d.getRotation());

    // System.out.println(goal.getX() + " " + goal.getY() + " " +
    // goal.getRotation().getDegrees());

    // path = AutoBuilder.pathfindToPose(goal, new
    // PathConstraints(Constants.maxChassisSpeed, Constants.maxAcceleration,
    // Units.degreesToRadians(Constants.maxChassisTurnSpeed),
    // Units.degreesToRadians(Constants.maxChassisTurnSpeed)));
